     ...
     await session.withTransaction(async () => {
      ...
      const experiment = await runSummary.findOne({
       ...
       for (let pbs_id in data) {
          ...
          const [manifestRestart, routineList, filePath] = await Promise.all([
            db.collection("job_manifest_restart").insertOne(
                getFields(job_data["manifests/restart.yaml"]), { session }),
            db.collection("routine_list").insertOne(
                getRoutineList(job_data["access-om2.out"]), { session }),
            db.collection("job_file_path").insertOne(
          ...
          if (experiment) {
            ...
            const [config, diag, env, manifestExec, manifestInput, namelist] = await Promise.all([
                db.collection("job_config").findOne({ "_id": experiment.config}),
                db.collection("job_diag").findOne({"_id": experiment.diag}),
                db.collection("job_env").findOne({"_id": experiment.env}),
                db.collection("job_manifest_exec").findOne({"_id": experiment.manifest_exec}),
                db.collection("job_manifest_input").findOne({"_id": experiment.manifest_input}),
                db.collection("namelist").findOne({"_id": experiment.namelist})
            ])
            ...
            const [config, diag, env, manifestExec, manifestInput, namelist] = await Promise.all([
                db.collection("job_config").insertOne(getFields(job_data["config.yaml"]), 
                { session }),
                db.collection("job_diag").insertOne(getFields(job_data["ice_diag.d"]),
                { session }),
                db.collection("job_env").insertOne(getFields(job_data["env.yaml"]),
                { session }),
                db.collection("job_manifest_exec")
                .insertOne(getFields(job_data["manifests/exe.yaml"]), { session }),
                db.collection("job_manifest_input")
                .insertOne(getFields(job_data["manifests/input.yaml"]), { session }),
              ...
              await db.collection("run_summary").insertOne({
                ...
                diag: diag.insertedId, 
                env: env.insertedId, 
                namelist: namelist.insertedId,
                manifest_exec: manifestExec.insertedId,
                manifest_input: manifestInput.insertedId,
